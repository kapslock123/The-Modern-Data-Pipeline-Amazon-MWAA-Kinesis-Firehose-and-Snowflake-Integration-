...
task_customer_landing_to_processing = BashOperator(
 ...
 bash_command='aws s3 mv s3://snowflakeairflow/firehose/customers/landing/ s3://snowflakeairflow/firehose/customers/processing/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
task_customers_processing_to_processed = BashOperator(
 task_id="customer_processing_to_processed",
 bash_command='aws s3 mv s3://snowflakeairflow/firehose/customers/processing/{0}/ s3://snowflakeairflow/firehose/customers/processed/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
task_orders_landing_to_processing = BashOperator(
 task_id="orders_landing_to_processing",
 bash_command='aws s3 mv s3://snowflakeairflow/firehose/orders/landing/ s3://snowflakeairflow/firehose/orders/processing/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
task_orders_processing_to_processed = BashOperator(
 task_id="orders_processing_to_processed",
 bash_command='aws s3 mv s3://snowflakeairflow/firehose/orders/processing/{0}/ s3://snowflakeairflow/firehose/orders/processed/{0}/ --recursive'.format(batch_id),
 dag=dag
)
